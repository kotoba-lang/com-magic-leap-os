(ns magic_leap_os.main
  "Kotodama WASM entrypoint for the Magic_leap_os clean-room actor (L4) — Clojure port.
  ...
  facts (`magic_leap_os.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "magic_leap_os")
(def tier "L4")
...
(def entity-specs
  [{:entity "Anchor"   :plural "anchors"   :id-prefix "magiclea_anc"
    :fields [:sceneId :x :y :z :persistent]              :required [:x :y]
    :coerce {:x :float :y :float :z :float}                       :refs {:sceneId "Scene"}}
   {:entity "Scene"    :plural "scenes"    :id-prefix "magiclea_sce"
    :fields [:name :deviceId :meshRef] :required [:name]
    :coerce {}                       :refs {:deviceId "Device"}}
   {:entity "Mesh"     :plural "meshes"     :id-prefix "magiclea_mes"
    :fields [:sceneId :vertexCount :contentRef]        :required [:vertexCount]
    :coerce {:vertexCount :int}          :refs {:sceneId "Scene"}}
   {:entity "Session"  :plural "sessions"  :id-prefix "magiclea_ses"
    :fields [:deviceId :appId :startedAt]          :required [:startedAt]
    :coerce {}                       :refs {:deviceId "Device"}}
   {:entity "Asset"    :plural "assets"    :id-prefix "magiclea_ass"
    :fields [:sceneId :type :contentRef]          :required [:type]
    :coerce {}                       :refs {:sceneId "Scene"}}
   {:entity "Device"   :plural "devices"   :id-prefix "magiclea_dev"
...
(defn emit-facts
  "EAVT facts for one record: {\"magic_leap_os.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "magic_leap_os-compat" :tier tier :entities entities} 200])
...
(defn start! [] :magic_leap_os-compat/ready)
