(ns magic_leap_os.main-test
  "Contract + behavioral test for the magic_leap_os-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "magic_leap_os-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'magic_leap_os.main-test)]
